  ...
  return (
    <MarketingShell>
      ...
      <JsonLd data={{ "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
        { "@type": "ListItem", position: 1, name: "Industries", item: `${SITE_URL}/industries/cafes` },
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <p className="mt-5 max-w-2xl text-lg text-slate-600">{page.intro}</p>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2">
            {page.bullets.map((bullet) => <div key={bullet} className="flex gap-3 rounded-xl border border-slate-200 p-4"><CheckCircle className="mt-0.5 h-5 w-5 text-blue-600" /><span className="text-sm font-medium text-slate-700">{bullet}</span></div>)}
          </div>
        </div>
      </section>
      <section className="bg-slate-50 px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-6xl gap-5 md:grid-cols-3">
          {page.sections.map((section) => <div key={section.title} className="rounded-xl border border-slate-200 bg-white p-6"><h2 className="text-lg font-bold text-slate-950">{section.title}</h2><p className="mt-3 text-sm leading-6 text-slate-600">{section.body}</p></div>)}
        </div>
      </section>
      <section className="px-4 py-16 sm:px-6 lg:px-8">
