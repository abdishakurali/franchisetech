...
export default async function ZReportPage({ searchParams }: { searchParams?: Promise<{ date?: string }> }) {
  const { supabase, orgId, user, membership, currency } = await getKitchenOpsContext();
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between gap-4 flex-wrap print:hidden">
        <div>
          <h1 className="text-2xl font-semibold text-slate-950">Daily Z-Report</h1>
          <p className="text-sm text-slate-500">End-of-day cash close report.</p>
        </div>
        <div className="flex gap-3 items-center">
          <form className="flex gap-2">
            ...
            <button type="submit" className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm font-medium hover:bg-slate-50">Load</button>
          </form>
          <PrintButton />
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 print:border-0">
        <div className="flex flex-wrap gap-8 text-sm">
          <div><p className="text-slate-500">Business</p><p className="font-semibold text-slate-900">{((org as { name?: string | null } | null)?.name) ?? "franchisetech"}</p></div>
          <div><p className="text-slate-500">Report date</p><p className="font-semibold text-slate-900">{reportDate}</p></div>
          <div><p className="text-slate-500">Generated at</p><p className="font-semibold text-slate-900">{generatedAt}</p></div>
          <div><p className="text-slate-500">Generated by</p><p className="font-semibold text-slate-900">{generatedBy}</p></div>
        </div>
      </div>

      {/* KPI row */}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-slate-500">Transactions</CardTitle></CardHeader><CardContent className="text-2xl font-bold">{completedTx.length}</CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-slate-500">Net sales</CardTitle></CardHeader><CardContent className="text-2xl font-bold">{formatMoney(totalNet || (totalGross - totalVat), currency)}</CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-slate-500">VAT collected</CardTitle></CardHeader><CardContent className="text-2xl font-bold text-blue-600">{formatMoney(totalVat, currency)}</CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-slate-500">Gross sales excl. tips</CardTitle></CardHeader><CardContent className="text-2xl font-bold text-green-600">{formatMoney(grossExTips, currency)}</CardContent></Card>
      </div>
      {totalTips > 0 && (
